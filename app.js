...
MongoClient.connect('mongodb://127.0.0.1:27017/Series', function (err, db) {
    ...
    function rBG() {
        var rarbg = sendRequest('GET', 'https://torrentgalaxy.to/rss');
        rarbg.then(function (response) {
            xmlparse(response.text, function (err, result) {
                ...
                async.each(result, function (item) {
                    if (item.category[0].toLowerCase().indexOf('tv') > -1) {
    ...
    function getSubs(queryString) {
        var loginToken = "";
        os.api.login().then(function (token) {
            loginToken = token;

            os.api.search(loginToken, "rum", { query: "Ray.Donovan.S03E02.REPACK.1080p.HDTV.X264-DIMENSION" }).then(function (result) {
                debugger;
            });
        });
    }
